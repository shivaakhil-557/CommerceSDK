...
export default class NavigateBackToCustomerAddEditCommand extends SearchView.CustomerSearchExtensionCommandBase {
    ...
    constructor(context: IExtensionCommandContext<SearchView.ICustomerSearchToExtensionCommandMessageTypeMap>) {
        ...
        this.label = "Go back to create customer";
    ...
    protected execute(): void {
        ...
        if (!ObjectExtensions.isNullOrUndefined(customer)) {
            let navigationParam: Commerce.Client.Entities.CustomerDetailsNavigationParameters = new Commerce.Client.Entities.CustomerDetailsNavigationParameters(customer.AccountNumber);
            this.context.navigator.navigateToPOSView("CustomerDetailsView", navigationParam);
        }
    }
}