...
export default class ShowChangeDueClientRequestHandlerExt extends ShowChangeDueClientRequestHandler {
     ...
     */
    public async executeAsync(request: ShowChangeDueClientRequest): Promise<ClientEntities.ICancelableDataResult<ShowChangeDueClientResponse>> {

        // User could implement new business logic here to override the dialog.
        await MessageHelpers.ShowSuccessMessage(this.context, "Thank you");
        return Promise.resolve({ canceled: false, data: new ShowChangeDueClientResponse() });
    }
}
